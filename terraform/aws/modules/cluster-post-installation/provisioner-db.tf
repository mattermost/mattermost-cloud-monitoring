...
resource "aws_db_instance" "provisioner" {
  ...
  vpc_security_group_ids      = "[${data.aws_security_group.cec_to_postgress.id}]"
  ...
  depends_on = [
    "aws_security_group.cec_to_postgress"
  ]
}


output "provisioner_db_endpoint" {
  value = "${aws_db_instance.provisioner.endpoint}"
}

output "provisioner_db_port" {
  value = "${aws_db_instance.provisioner.port}"
}
