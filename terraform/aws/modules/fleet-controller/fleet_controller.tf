        ...
        template {
          ...
          spec {
            container {
              ...
              args              = ["hibernate", "--unlock", "--server=${var.provisioner_server}", "--thanos-url=${var.thanos_server}", "--group=${var.cws_group}", "--dry-run=false", "--max-users=${var.fleet_controller_max_users_filter}"]
              env {
